...
## 0. 최우선 기준 (사용자 지시 2026-09-15)
- 이 사이트의 핵심 목표는 **네이버에서 "골프투어", "태국골프투어", "베트남골프투어", "중국골프투어", "일본골프투어" 등 골프투어 키워드 검색 시 노출**되는 것이다. 모든 수정·보완은 이 기준으로 판단한다.
- 국가 페이지(/overseas/{slug})는 "{국가}골프투어"의 랜딩 페이지: 제목·H1·설명·본문 첫 문단에 "{국가} 골프투어"와 붙여 쓴 "{국가}골프투어"가 모두 들어가야 한다.
- 네이버 서치어드바이저 소유확인 메타(layout.tsx), Yeti 크롤러 허용(robots.ts), sitemap 제출 상태를 매번 확인한다.

## 1. 콘텐츠 원칙 (위반 = 즉시 수정)
- 허위 정보·과장 표현(최저가, 업계 1위 등) 금지. 실적·가격은 근거 있는 값만, 자체 집계는 단서 표기
- 허위 후기 금지. 예시 콘텐츠(견적서 목업, 상담 대화)에는 "예시" 라벨 필수
- **원가·대행비·마진 공개 표현 절대 금지** (대표 지시). "원가", "대행비", "까놓고", "거품" 단어를 grep으로 확인. 견적서 예시는 포함·불포함 내역만 보여준다
